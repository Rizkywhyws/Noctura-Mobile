...
class FeatureGrid extends StatelessWidget {
  ...
  const FeatureGrid({super.key, this.onLogTidurTap, this.onEducationTap});
...
class _FeatureCardState extends State<_FeatureCard>
  ...
  Widget build(BuildContext context) {
    ...
    final border =
        widget.isDark ? widget.borderColorDark : widget.borderColorLight;
    final titleColor =
        widget.isDark ? widget.titleColorDark : widget.titleColorLight;
    final effectiveIcon =
        widget.isDark ? widget.iconColorDark : widget.iconColor;
    final iconBg =
        widget.isDark ? widget.iconColorDark.withOpacity(0.18) : Colors.white;
    final subtitleColor = widget.isDark
        ? widget.subtitleColorDark.withOpacity(0.85)
        : widget.iconColor.withOpacity(0.70);

    return GestureDetector(
      // ── FIX: pastikan seluruh bounding box card bisa diklik,
      //        termasuk area transparan di dalam Stack/Positioned.
      ...
      onTap: () {
        ...
        debugPrint('=== FeatureCard tapped: ${widget.title}');
        widget.onTap?.call();
      },
